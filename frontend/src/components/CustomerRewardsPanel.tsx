...
function message(error: unknown) { if (error instanceof ApiError) { if (error.status === 404) return 'Cliente ou recompensa não encontrada.'; if (error.status === 409) return rewardConflictMessage(error.message); if (error.status === 401) return 'Sua sessão expirou. Entre novamente.' }; return 'Não foi possível carregar as recompensas do cliente.' }

function rewardConflictMessage(code: string) { return ({ CUSTOMER_REWARD_NOT_REDEEMABLE_YET: 'Esta recompensa ainda não pode ser resgatada.', CUSTOMER_REWARD_EXPIRED: 'Esta recompensa expirou.', CUSTOMER_REWARD_SELECTION_REQUIRED: 'É necessário selecionar uma recompensa antes do resgate.', CUSTOMER_REWARD_NOT_FOUND: 'Recompensa não encontrada ou não está mais disponível.', REWARD_TIER_MISMATCH: 'A recompensa selecionada não pertence à faixa permitida.', INSUFFICIENT_REWARD_FUND: 'Saldo insuficiente no fundo de recompensas.' } as Record<string, string>)[code] ?? 'Não foi possível concluir a operação.' }
