...
import type {
  ApproveCustomerRewardInput,
  DenyCustomerRewardInput,
  SelectCustomerRewardInput,
...
const REWARD_PROGRESS_INTERVAL =
  20;
...
type RewardRedemptionTimingType =
  | "IMMEDIATE"
  | "NEXT_PURCHASE"
  | "NEXT_PURCHASE_DAY";
...
export function calculateRewardTierFromCost(
  costAmount: number
): RewardTierType {
  if (
    costAmount <= 10
...
function getSuggestedRewardTier(
  rewardNumber: number,
  regularity: number
): RewardTierType {
  const position =
  ...
  if (
    ...
    return HIGH_REGULARITY_SEQUENCE[
      position
    ]!;
  }
  ...
  if (
    ...
    return MEDIUM_REGULARITY_SEQUENCE[
      position
    ]!;
  }


  return LOW_REGULARITY_SEQUENCE[
    position
  ]!;
}
 ...
 */
export async function createPendingCustomerRewardsForProgress(
  ...
  const rows:
    Prisma.CustomerRewardCreateManyInput[] = [];


  for (
    let rewardNumber = firstRewardNumber;
    rewardNumber <= lastRewardNumber;
    rewardNumber += 1
  ) {
    const milestone =
      rewardNumber *
...
async function ensureEmployeeBelongsToCompany(
  tx: Prisma.TransactionClient,
  companyId: number,
  employeeId: number
): Promise<void> {
...
function resolveExpirationDate(
  ...
  timing: RewardRedemptionTimingType,
  approvedAt: Date
): Date | null {
  if (!expiresOn) {
    return null;
...
export async function listPendingCustomerRewards(
  companyId: number
) {
  const company =
...
export async function approveCustomerReward(
  companyId: number,
  employeeId: number,
  customerRewardId: bigint,
  data: ApproveCustomerRewardInput
) {
  return prisma.$transaction(
    async (
      tx
    ) => {
      await ensureEmployeeBelongsToCompany(
        tx,
        companyId,
        employeeId
      );
      ...
      const pendingReward =
        await tx.customerReward.findFirst({
          where: {
            idCustomerReward:
              customerRewardId,
      ...
      const result =
        await tx.customerReward.findUnique({
          where: {
            idCustomerReward:
              customerRewardId,
          },
...
export async function denyCustomerReward(
  companyId: number,
  employeeId: number,
  customerRewardId: bigint,
  data: DenyCustomerRewardInput
) {
  return prisma.$transaction(
    async (
      tx
...
async function expireCustomerRewardInTransaction(
  tx: Prisma.TransactionClient,
  customerRewardId: bigint,
  now: Date
): Promise<boolean> {
  ...
  for (
    ...
    await prisma.$transaction(
      async (
        ...
        await expireCustomerRewardInTransaction(
          tx,
          customerReward.idCustomerReward,
          now
        );
      }
    );
...
export async function selectCustomerReward(
  companyId: number,
  customerRewardId: bigint,
  data: SelectCustomerRewardInput
) {
  return prisma.$transaction(
    async (
      tx
      ...
      if (
        customerReward.Reward_idReward !==
        null
      ) {
        throw new Error(
...
async function canRedeemCustomerReward(
  tx: Pick<
...
export async function redeemCustomerReward(
  ...
  return prisma.$transaction(
    async (
      ...
      if (
        customerReward.rewardType === "CHOICE" &&
